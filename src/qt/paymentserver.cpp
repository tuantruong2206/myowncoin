...
const char* BIP71_MIMETYPE_PAYMENT = "application/nilabit-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/nilabit-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/nilabit-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the nilabit: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(bitcoin_IPC_PREFIX, Qt::CaseInsensitive)) // nilabit: URI
        {
            ...
            if (GUIUtil::parsenilabitURI(arg, &r) && !r.address.isEmpty())
            {
                CnilabitAddress address(r.address.toStdString());
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click nilabit: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start nilabit: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling nilabit: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in nilabit: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(bitcoin_IPC_PREFIX, Qt::CaseInsensitive)) // nilabit: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parsenilabitURI(s, &recipient))
            {
                CnilabitAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Nilabit address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CnilabitAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom Nilabit addresses are not supported
        ...
        // Nilabit amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(nilabitUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
