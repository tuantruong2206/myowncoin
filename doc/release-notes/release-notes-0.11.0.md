Nilabit Core version 0.11.0 is now available from:

  <https://nilabit.org/bin/nilabit-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer NLB/kB fee to be
...
related to the blockchain in the Nilabit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Nilabit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Nilabit Core will 
